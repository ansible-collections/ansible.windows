...
namespace ansible_collections.ansible.windows.plugins.module_utils._CredentialManager
{
        ...
        public static List<Credential> EnumerateCredentials(string filter)
        {
            ...
            try
            {
                for (int i = 0; i < (int)count; i++)
                {
                    ...
                    NativeHelpers.CREDENTIAL raw = (NativeHelpers.CREDENTIAL)Marshal.PtrToStructure(
                        pCred, typeof(NativeHelpers.CREDENTIAL));
                    results.Add((Credential)raw);
                }
            }
            finally
            {
