...
Function ConvertTo-CredentialAttribute {
    ...
    $converted_attributes = [System.Collections.Generic.List[CredentialAttribute]]@()
    foreach ($attribute in $Attributes) {
        $new_attribute = New-Object -TypeName CredentialAttribute
...
$persistence = switch ($persistence) {
    "local" { [CredentialPersist]::LocalMachine }
    "enterprise" { [CredentialPersist]::Enterprise }
}
...
$existing_credential = [Credential]::GetCredential($name, $type)
...
if ($state -eq "absent") {
    ...
    if ($null -eq $existing_credential) {
        $new_credential = New-Object -TypeName Credential
    ...
    if ($module.CheckMode) {
        ...
        $new_credential = [Credential]::GetCredential($name, $type)
