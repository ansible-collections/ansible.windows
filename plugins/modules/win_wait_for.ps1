...
Function Get-PortConnection($hostname, $port) {
    $connections = @()
    ...
    if ($hostname -eq "0.0.0.0") {
        ...
        $active_connections = $conn_info.GetActiveTcpConnections() |
            Where-Object { $_.LocalEndPoint.Address -eq $hostname -and $_.LocalEndPoint.Port -eq $port }
    }
